...
resource "azurerm_linux_virtual_machine_scale_set" "ubuntu18_virtual_machine_scale_set" {
  count = var.create_ubuntu18_scale_set ? 1 : 0
  lifecycle {
    ignore_changes = [instances, tags, automatic_os_upgrade_policy, extension]
  }
  name                            = "${var.resource_prefix}-ubuntu18-vmss"
  resource_group_name             = data.terraform_remote_state.resources_state.outputs.resource_group_name
  location                        = data.terraform_remote_state.resources_state.outputs.resource_group_location
  sku                             = "Standard_D2_v3"
  instances                       = 1
  admin_username                  = "builderAdmin"
  admin_password                  = data.terraform_remote_state.resources_state.outputs.build_agent_secret
  source_image_id                 = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${data.terraform_remote_state.resources_state.outputs.resource_group_name}/providers/Microsoft.Compute/galleries/${var.shared_image_gallery_name}/images/ubuntu1804"
  disable_password_authentication = false
  overprovision                   = false
  upgrade_mode                    = "Manual"
  single_placement_group          = false
